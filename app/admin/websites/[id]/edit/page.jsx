...
export default function EditWebsite() {
  ...
  useEffect(() => {
    async function fetchWebsite() {
      ...
      if (res.ok) {
        ...
        const website = data.find((w) => w.id === params.id)
        if (website) {
          setForm({
            name: website.name || "",
            original_name: website.original_name || "",
            url: website.url || "",
          ...
          if (website.logo_path) {
            setLogoPreview(
              `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/logos/${website.logo_path}`
            )
          }
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/websites/${params.id}`, {
        ...
        body: JSON.stringify({ ...rest, logo_path }),
      })
  ...
  return (
    <div className="max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-white mb-6">Edit Website</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-white/30 transition-colors resize-none"
          />
        </div>
